...
public class Car implements Runnable{
	...
	if (cartype==REDCAR) {
            if (inFront.getX() - xpos > 100) {
                xpos += 4;
                if (xpos >= tunnelXLeft & xposOld < tunnelXLeft) controller.entraIzquierda();
		...
		else if (xpos >= tunnelXRight &&  xposOld < tunnelXRight) controller.dejaDerecha();
            }
	} else {
            if (xpos-inFront.getX() > 100) {
                xpos -= 4;
                if (xpos <= tunnelXRight && xposOld > tunnelXRight) controller.entraDerecha();
		...
		else if (xpos <= tunnelXLeft && xposOld > tunnelXLeft) controller.dejaIzquierda();
            }
	}
    }
